type TFakeContact = typeof fakeContacts[0]
const fakeContacts = [
...
function upperCaseSecondLetter(text: string) {
  return text
    .split(' ')
    .map((word) =>
      word
...
function changeSign(num: number) {
  if (num > 0) {
...
function doubleNumbers(str: string) {
  return str
    .split('')
...
function validatePassword(pas: string) {
  const specSymbols = '!@#$%^&*';
  switch (true) {
    case pas.includes(' '):
...
function findUniqueNumber(array: number[]) {
...
function camelCaseToGoogleStyle(array: string[]) {
...
function updateById(contacts: typeof fakeContacts, id: number, newInfo: Partial<TFakeContact>) {
...
function getNames(contacts: TFakeContact[]) {
  return contacts.map((c) => c.name);
}
console.log(getNames(fakeContacts)); // ['Bob', 'Ben', 'Eva', 'Max', 'Sam']

function addToObject(object: { [key: string]: string }, property: string, value: string) {
